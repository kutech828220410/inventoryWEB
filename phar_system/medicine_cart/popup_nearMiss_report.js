...
function set_main_report_display() {
    ...
    if(Array.isArray(nearMiss_report)) {
        ...
        if(temp_cpoe_data.length > 0 || temp_note_data.length > 0) {
            nearMiss_report.forEach(element => {
                if(element.medCpoe) {
...
async function popup_nearMiss_report_div_open() {
    ...
    if(return_data.Code == 200) {
        ...
        // nearMiss_report = split_nearMiss_data_and_recombine(nearMiss_report, disp_id);
        let temp_object_arr = split_nearMiss_data_and_recombine(nearMiss_report, disp_id);
        let temp_a_arr = sort_by_nurnum_bednum(temp_object_arr.a);
        let temp_b_arr = sort_by_nurnum_bednum(temp_object_arr.b);

        nearMiss_report = [...temp_a_arr, ...temp_b_arr];
...
function split_nearMiss_data_and_recombine(arr, disp_id) {
    let same_arr = arr.filter(item => item.disp_id == disp_id);
    let nusame_arr = arr.filter(item => item.disp_id != disp_id);

    return {a:same_arr, b:nusame_arr};
}
...
function sort_by_nurnum_bednum(arr) {
    ...
    temp_arr = temp_arr.sort((a, b) => {
        const nA = (a?.nurnum ?? "").toString();
        const nB = (b?.nurnum ?? "").toString();
