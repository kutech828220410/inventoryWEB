...
function set_main_list_display() {
  ...
  code_mode_data.forEach(element => {
    ...
    let temp_actualIssuedQuantity = 0;
    let temp_do_actualIssuedQuantity = 0;
    element["RES_UNIT"].forEach(e => {
      // 計算請領總量
      temp_actualIssuedQuantity += +e.actualIssuedQuantity;

      let unit_div = document.createElement("div");
      unit_div.classList.add("unit_div");
      unit_div.setAttribute("GUID", e.GUID);

      if(e.actualIssuedQuantity == "" || e.actualIssuedQuantity == undefined) {
        unit_div.innerHTML = `<div>修改</div><div>${e.destinationStoreType}&nbsp;&nbsp;0 / ${e.issuedQuantity}</div>`;
      } else {
        unit_div.innerHTML = `<div>修改</div><div>${e.destinationStoreType}&nbsp;&nbsp;${e.actualIssuedQuantity} / ${e.issuedQuantity}</div>`;
      }

      if(e.state == "已過帳") {
        unit_div.classList.add("unit_done");
        temp_do_actualIssuedQuantity += +e.actualIssuedQuantity;
...
function set_main_list_table_display() {
  ...
  unit_mode_data.forEach(element => {
    ...
    let mcit_title = document.createElement("div");
    mcit_title.classList.add("mcit_title");
    mcit_title.innerHTML = element.destinationStoreType;
...
async function set_list_result_and_filter() {
  ...
  if(temp_search_condition.req_unit != "all") { 
    post_result_data = post_result_data.filter(item => item["destinationStoreType"] == temp_search_condition.req_unit);
  }
