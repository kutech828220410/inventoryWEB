...
async function confirm_popup_upload()
{
    ...
    {   
        ...
        const confirmResult = confirm("確定上傳盤點單?");
        if (confirmResult)
        {     
            ...
            await inventory_excel_upload(upload_input.files[0], IC_NAME_input_popup.value, get_logedName(), PON_input_popup.value);
...
function get_title_popup_upload()
{
    ...
    My_Div.Set_Text(title_text ,"上傳盤點單" , TextAlignEnum.CENTER , "24px", true,"微軟正黑體","white");
...
function get_box_popup_upload()
{
    ...
    IC_NAME_input.placeholder = '請輸入盤點名稱';
    ...
    PON_input.placeholder = '請輸入預設盤點人';
...
//#endregion
async function inventory_excel_upload(file, IC_NAME, CT, DEFAULT_OP) 
{
  const formData = new FormData();
  formData.append('file', file);
  formData.append('IC_NAME', IC_NAME);
  formData.append('CT', CT);
  formData.append('DEFAULT_OP', DEFAULT_OP);
  console.log("post_data [inventory_excel_upload]",formData);
  try {
    const response = await fetch(`${api_ip}api/inventory/excel_upload`, {
      method: 'POST',
      body: formData
    });
