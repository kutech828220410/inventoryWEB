...
// }
function creat_poprow_div(_index , Med) 
{
    ...
    const popdrug_div = get_popdrug_div(_index, Med);
    row_div.appendChild(popdrug_div);
    if(device == DeviceType.MOBILE) 
    {
...
function get_popdrug_div(_index, item)
{
    const Block1_div = document.createElement("div");
    My_Div.Init(Block1_div, 'Block1_div',`Block1_div${_index}`, '100%', '100%', '');
    My_Div.Set_Block(Block1_div, DisplayEnum.FLEX, FlexDirectionEnum.COLUMN, JustifyContentEnum.TOP);

    //藥品資訊
    const drugInfo_div = document.createElement("div");
    My_Div.Init(drugInfo_div, 'drugInfo_div',`drugInfo_div${_index}`, '130px', '100%', '');
    My_Div.Set_Block(drugInfo_div, DisplayEnum.FLEX, FlexDirectionEnum.ROW, JustifyContentEnum.LEFT);
