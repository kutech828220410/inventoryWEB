...
{
    const med_AREA_text = document.querySelector('#med_AREA_text_popup_input');
    med_AREA_text.innerText = `區域 : ${Med.AREA}`;
    if(Med.AREA == null || Med.AREA == "")
    {
        med_AREA_text.innerText = `區域：無`;
    }
    else med_AREA_text.innerText = `區域${Med.AREA}`;
    //儲位IP顯示
    const med_IP_text = document.querySelector('#med_IP_text_popup_input');
    med_IP_text.innerText = `IPIP${Med.CODE}`;
...
function get_title_popup_input()
{
    ...
    My_Div.Init(title_div, 'title_div_popup_input','title_div_popup_input', '100%', '110px', '');
    ...
    My_Div.Init(title_others, 'title_others_popup_input','title_others_popup_input', '100%', '60px', '');
    ...
    My_Div.Set_Block(title_control_block, DisplayEnum.FLEX, FlexDirectionEnum.COLUMN, JustifyContentEnum.CENTER);
    ...
    const IP_info = document.createElement('div');
    My_Div.Init(IP_info, 'IP_info_popup_input','IP_info_popup_input', '85%',"100%" ,'');
    My_Div.Set_Block(IP_info, DisplayEnum.FLEX, FlexDirectionEnum.COLUMN, JustifyContentEnum.TOP);

    const med_CODE_IP_block = document.createElement('div');
    My_Div.Init(med_CODE_IP_block, 'med_CODE_IP_block_popup_input','med_CODE_IP_block_popup_input', '100%',"100%",'');
    My_Div.Set_Block(med_CODE_IP_block, DisplayEnum.FLEX, FlexDirectionEnum.ROW,JustifyContentEnum.LEFT)

    const med_AREA_text = document.createElement('div');
    My_Div.Init(med_AREA_text,'med_AREA_text_popup_input','med_AREA_text_popup_input', '30%',"30px");
    My_Div.Set_Text(med_AREA_text ,"區域 : XXXXX" , TextAlignEnum.CENTER , "16px", true,"微軟正黑體","black");
    med_AREA_text.style.border = "1px solid";
    med_AREA_text.style.marginLeft = "5px";
    ...
    const med_IP_text = document.createElement('div');
    My_Div.Init(med_IP_text,'med_IP_text_popup_input','med_IP_text_popup_input', '70%',"30px");
    My_Div.Set_Text(med_IP_text ,"IPIPXXXXX" , TextAlignEnum.RIGHT , "22px", true,"微軟正黑體","pink");
    med_IP_text.style.marginRight = "5px";

    med_CODE_IP_block.appendChild(med_AREA_text);
    med_CODE_IP_block.appendChild(med_IP_text);
    ...
    IP_info.appendChild(med_CODE_IP_block);


    
    title_others.style.borderBottom = "2px solid";
    title_others.appendChild(IP_info);
...
function get_row_popup_input(BarCode)
{
    ...
    // const drug1 = get_MED_popup_input();
...
function get_drug_popup_input()
{
    const drug1 = document.createElement('div');
    My_Div.Init(drug1, 'drug1_popup_input','drug1_popup_input', '100%','60px');
    My_Div.Set_Block(drug1, DisplayEnum.FLEX, FlexDirectionEnum.ROW, JustifyContentEnum.LEFT);

    const MED_info_div = document.createElement('div');
    My_Div.Init(MED_info_div, 'MED_info_div_popup_input','MED_info_div_popup_input', '100%','70px');
    My_Div.Set_Block(MED_info_div, DisplayEnum.FLEX, FlexDirectionEnum.COLUMN, JustifyContentEnum.CENTER);
    MED_info_div.style.marginTop = "20px";

    const CODE_SKDIACODE_div = document.createElement('div');
    My_Div.Init(CODE_SKDIACODE_div, 'CODE_SKDIACODE_div_popup_input','CODE_SKDIACODE_div_popup_input', '100%','70px');
    My_Div.Set_Block(CODE_SKDIACODE_div, DisplayEnum.FLEX, FlexDirectionEnum.ROW, JustifyContentEnum.CENTER);
    const med_CODE_text = document.querySelector('#med_CODE_text_popup_input');
    med_CODE_text.innerText = `藥碼 : ${Med.CODE}`;
    const med_SKDIACODE_text = document.querySelector('#med_SKDIACODE_text_popup_input');
    med_SKDIACODE_text.innerText = `料號 : ${Med.SKDIACODE}`;
    ...
    My_Div.Init(NAME_div, 'NAME_div_popup_input','NAME_div_popup_input', '100%','70px');
    My_Div.Set_Block(NAME_div, DisplayEnum.FLEX, FlexDirectionEnum.COLUMN, JustifyContentEnum.CENTER);
    const med_eng_name_text = document.querySelector('#med_eng_name_text_popup_input');
    if(Med.NAME != null)med_eng_name_text.innerText = `(英) : ${Med.NAME}`;
    const med_cht_name_text = document.querySelector('#med_cht_name_text_popup_input');
    if(Med.CHT_NAME != null) med_cht_name_text.innerText = `(中) : ${Med.CHT_NAME}`;

    CODE_SKDIACODE_div.appendChild(med_CODE_text);
    CODE_SKDIACODE_div.appendChild(med_SKDIACODE_text);
    NAME_div.appendChild(med_eng_name_text);
    NAME_div.appendChild(med_cht_name_text);
    MED_info_div.appendChild(CODE_SKDIACODE_div);
    MED_info_div.appendChild(NAME_div);
    MED1.appendChild(MED_info_div);
    MED1.appendChild(trashBox_SVG);
...
function get_block1_popup_input(Barcode)
{
    ...
    // block1_barcode_div.appendChild(med_CODE_text);
