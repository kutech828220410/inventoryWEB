...
async function get_search_result() {
        ...
        switch (select_med_kind.value) {
            case "code":
                temp_data = data_information.filter((e) => {
                    return e["CODE"].toUpperCase().includes(select_med_input.value.toUpperCase());
                });
                data_information = temp_data;
                break;
            case "name":
                temp_data = data_information.filter((e) => {
                    return e["NAME"].toUpperCase().includes(select_med_input.value.toUpperCase());
                });
                data_information = temp_data;
                break;
            default:
    ...
    if(select_patient_input.value != "") {
        ...
        switch (select_patient_kind.value) {
            ...
            case "med_bag_num":
                temp_data = data_information.filter((e) => {
                    return e["MED_BAG_NUM"].toUpperCase().includes(select_patient_input.value.toUpperCase());
                });
                data_information = temp_data;
                break;
            default:
